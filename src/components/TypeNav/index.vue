...
export default {
  ...
  mounted() {
    ...
    // this.$store.dispatch('getCategoryList');   //由于home页面与其它页面频繁切换时，该请求会被频繁执行，从而造成大量重复访问，因此，放在App.vue中
  ...
  computed: {
    ...
    ...mapState({
      //state 其实是大仓库中的数据有home中的state和search当中的state
      categoryList: (state) => {
        // console.log(state);
        // 返回大仓库当中的home中的categoryList数据
  ...
  methods: {
    ...
    //throttle回调函数别用箭头函数，可能出现this指向问题
    ...
    //进行路由跳转的方法
    //由于大量使用<router-link>需要创建许多组件实例，会造成性能损耗大量内存占用，会造成页面卡顿。最好的解决方案：编程式导航 + 事件委派
    //存在一些问题：事件委派，是把全部的子节点(h3、dt、dl、em)的事件委派给父亲节点
    //点击a标签的时候，才会进行路由跳转(怎么能确定点击的一定是a标签)
    //存在另一个问题，即使你能确定点击的是a标签，如何区分是一级、二级、三级分类的标签
    //第一个问题解决方案：把子节点当中a标签，加上自定义属性data-categoryName，其余的子节点是没有的
    //进行路由跳转的回调函数
    goSearch(event) {
